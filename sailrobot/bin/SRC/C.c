...
/*
 * The C_FUNCTION function is used when the distance is defined by a function
 * (e.g. the Euclidean distance function). In order to speed up the
 * computations the following algorithm used:
 * 
 * (1) If (Na,Nb) is an edge on the current tour, then its distance is available
 * in either the field PredCost or SucCost.
 * 
 * (2) A hash table (CacheVal) is consulted to see if the distance has been
 * stored.
 * 
 * (3) Otherwise the distance function is called and the distance computed is
...
int c_ATT(Node * Na, Node * Nb)
{
    if (Forbidden(Na, Nb))
        return Na->Pi + Nb->Pi;
    int dx = (int) (ceil(0.31622 * fabs(Na->X - Nb->X))), dy =
        (int) (ceil(0.31622 * fabs(Na->Y - Nb->Y)));
...
int c_CEIL_2D(Node * Na, Node * Nb)
{
    if (Forbidden(Na, Nb))
        return Na->Pi + Nb->Pi;
    int dx = (int) ceil(fabs(Na->X - Nb->X)), dy =
        (int) ceil(fabs(Na->Y - Nb->Y));
    return (dx > dy ? dx : dy) * Precision + Na->Pi + Nb->Pi;
}
...
int c_CEIL_3D(Node * Na, Node * Nb)
{
    if (Forbidden(Na, Nb))
        return Na->Pi + Nb->Pi;
    int dx = (int) ceil(fabs(Na->X - Nb->X)), dy =
        (int) ceil(fabs(Na->Y - Nb->Y)), dz =
        (int) ceil(fabs(Na->Z - Nb->Z));
    if (dy > dx)
        dx = dy;
    if (dz > dx)
        dx = dz;
...
int c_EUC_2D(Node * Na, Node * Nb)
{
    if (Forbidden(Na, Nb))
        return Na->Pi + Nb->Pi;
    int dx = (int) (fabs(Na->X - Nb->X) + 0.5), dy =
        (int) (fabs(Na->Y - Nb->Y) + 0.5);
    return (dx > dy ? dx : dy) * Precision + Na->Pi + Nb->Pi;
}
...
int c_EUC_3D(Node * Na, Node * Nb)
{
    if (Forbidden(Na, Nb))
        return Na->Pi + Nb->Pi;
    int dx = (int) (fabs(Na->X - Nb->X) + 0.5), dy =
        (int) (fabs(Na->Y - Nb->Y) + 0.5), dz =
        (int) (fabs(Na->Z - Nb->Z) + 0.5);
    if (dy > dx)
        dx = dy;
    if (dz > dx)
        dx = dz;
...
int c_FLOOR_2D(Node * Na, Node * Nb)
{
    if (Fixed(Na, Nb))
        return Na->Pi + Nb->Pi;
    int dx = (int) floor(fabs(Na->X - Nb->X)), dy =
        (int) floor(fabs(Na->Y - Nb->Y));
    return (dx > dy ? dx : dy) * Precision + Na->Pi + Nb->Pi;
}
...
int c_FLOOR_3D(Node * Na, Node * Nb)
{
    if (Fixed(Na, Nb))
        return Na->Pi + Nb->Pi;
    int dx = (int) floor(fabs(Na->X - Nb->X)), dy =
        (int) floor(fabs(Na->Y - Nb->Y)), dz =
        (int) floor(fabs(Na->Z - Nb->Z));
    if (dy > dx)
        dx = dy;
    if (dz > dx)
        dx = dz;
